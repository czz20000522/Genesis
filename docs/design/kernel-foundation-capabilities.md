...
The first local skill store is the configured skill-root set, defaulting to the current user's Genesis-owned root at `~/.genesis/skills` followed by the shared agent skill root at `~/.agents/skills`. Explicit `GENESIS_SKILL_ROOTS` entries and repeated `-skill-root` flags are operator setup surfaces and are ordered before the default roots. Effective roots are de-duplicated while preserving the first occurrence, so an explicit root can intentionally take priority without being scanned twice. `-disable-default-skill-roots` is a smoke/dev escape hatch for focused sessions; it is not the product default and must not be used to make installed user skills disappear silently.
...
- round-budget pause: a turn-level control stop;
- storm guard: a tool-result-level model observation.

When a turn starts, the kernel mints a `BudgetLease` from normalized runtime
budget policy. The first slice of this lease owns the model tool-round
execution budget and its configured ceiling. Unset or non-positive model
tool-round budgets normalize to the kernel default; they do not mean unlimited.
The lease is inspectable through runtime projections but remains absent from
model-visible tool schemas.

When a provider step returns tool calls after the effective tool-round execution
budget is exhausted, the kernel does not execute that over-budget batch and
does not record it as admitted `tool.call` evidence. It records `turn.paused`
with:

- status: `paused`;
- reason: `tool_loop_round_budget_exhausted`;
- completed tool-round count;
- effective `BudgetLease` values;
- paused timestamp generated by the kernel.

`BudgetLease` is an execution budget. It is intentionally separate from hard
safety and projection caps such as output truncation, request body caps,
provider response body caps, provider retry/repair attempt caps, shell
...
Continuation is intentionally routed through ordinary `turn.submit` in the
same session. The shell, application connector, or desktop UI may label the user
action as "continue", but it only submits user intent; it does not own recovery.
Provider context for the continuation includes a kernel-generated conversation
history fragment describing the paused frontier and the committed tool
exchanges. It does not replay hidden ids or audit refs into the model-visible
surface.

Storm guard runs after normal validation, authorization, and execution result
classification, but before committing the next effect when the guard can decide
from already-known state. The initial signatures are intentionally narrow:

- repeated failure signature: provider batch shape plus each result's tool,
  status, and structured error code when present. Raw arguments, credentials,
  paths, and long stderr are not part of the signature;
- repeated write-like success signature: exact registered tool plus normalized
  effect signature for tools with trusted `workspace_write`,
  `kernel_state_write`, `process_start`, or `external_side_effect` access plans.
  For `shell_exec`, Phase A uses exact normalized command plus resolved cwd only
  when the access plan is trusted write-like; it does not attempt semantic shell
  equivalence.

After the repeated-failure threshold, the kernel appends the ordinary
`tool.result` but augments the model-visible payload with a `loop_guard`
directive. The original error remains visible so the model can reason about the
failure taxonomy. After the repeated-success threshold for write-like effects,
the kernel returns a guarded, non-executed `tool.result` and skips the new
effect. This is not permission denial, not sandbox denial, and not command
failure; it is a loop-control observation.

Reference alignment:

- Reasonix keeps `maxSteps` as a resumable guard after preserving session
  state. Genesis intentionally expresses that as kernel `turn.paused` evidence
  rather than a plain error string.
- Reasonix's storm breaker returns a model-facing loop-guard message after
  repeated same-error failures and blocks repeated write-like successes before
  another write. Genesis keeps the same behavior class but records typed tool
  results and turn facts.
- Codex separates model-actionable tool errors from fatal infrastructure errors
  through `RespondToModel` versus fatal paths. Genesis follows the same
  taxonomy: guard feedback is model-visible tool observation; only real
  infrastructure failure fails the turn.

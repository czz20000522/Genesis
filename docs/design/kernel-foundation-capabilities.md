...
When a provider step returns tool calls after the configured tool-round budget
is exhausted, the kernel does not execute that over-budget batch and does not
record it as admitted `tool.call` evidence. It records `turn.paused` with:

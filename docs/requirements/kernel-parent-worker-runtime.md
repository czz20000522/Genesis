...
- worker 的 role、model profile、provider route、工具授权、上下文策略和并发上限通过显式配置绑定；
...
- worker invocation 复用现有 `AgentInvocation` 准入语义，工具能力必须是 parent grant 的子集；
- provider 或 model profile 的并发限制参与调度，本地小模型可以配置为默认轻量 worker，但并非内核强制要求；
- parent 可将任务组织为 task graph，独立节点可以并行执行，纠缠节点按依赖顺序执行，阻塞节点以结构化状态返回；
- worker 的中间 transcript、工具 trace、provider 诊断不会自动并入 parent 对话历史；
...
- 由 parent 通过内核准入创建；
- 只能执行一个有边界的任务；
- 使用绑定 role 的 model profile、工具授权和上下文策略；
- 不能创建子 worker；
- 返回终态结果、阻塞原因或失败分类。
...
Role binding 将一个 worker role 绑定到一个 model profile、一组工具授权、上下文策略和调度限制。Role label 本身不授予权限；权限来自内核准入后的 capability grant。
...
Task graph 是 parent 生成并由内核记录的通用工作结构。节点可以引用 worker invocation、review invocation 或 manual unblock action；边表示依赖关系。Task graph 不是业务流程引擎，不解释业务语义，只提供状态、依赖、恢复和投影。
...
- 不把 task graph 扩展成通用 workflow engine、队列系统或应用任务管理系统。
...
- parent 可提交任务图；
- 内核记录节点、依赖、状态、invocation link 和失败分类；
- 调度遵守 role/profile/provider 并发上限；
- 支持恢复未完成图的可见状态。
...
3. 一个 role binding 可以绑定 model profile、工具授权、上下文策略和并发上限。
4. Parent 创建 worker 时只能选择已配置 role binding。
5. Worker invocation 的工具授权必须经过内核校验，并且不能超过 parent grant。
6. Worker 不能创建 worker；任何递归 delegation 请求都被拒绝为权限错误。
7. 本地单并发模型可以声明 `max_parallel=1`，调度不会并发占用该 profile。
8. 云端 provider/profile 可以声明更高并发，并由 task graph 调度利用。
9. Worker 的中间 transcript 和工具 trace 不会自动进入 parent conversation projection。
10. Parent final answer 可以引用 worker terminal result 和 review result。
11. Task graph replay 后能恢复节点状态、依赖关系、已完成 invocation 和阻塞原因。
12. Provider credential、raw prompt、sandbox profile、permission profile 和内部 kernel id 不进入 model-visible role binding schema。

...
- worker 的 role、model profile、provider route、预设工具集合、上下文策略和并发上限通过显式配置绑定；未写 role 上限时默认 6；
- parent binding 还定义可存活子代理总量；未写 `max_children` 时默认 24；
...
Role binding 将一个 worker role 绑定到一个 model profile、一组预设工具、上下文策略和调度限制。Role label 本身不授予权限；权限来自内核准入后的 capability grant。允许同时存在多个相同 role 的 worker invocation；它们是不同 agent 实例，受 role/profile/provider 并发上限约束。`max_parallel` 未配置时是 6；operator 可以为独占本地模型显式设为 1。

### Parent Profile

Parent profile 定义 user-facing parent agent 使用的模型配置和可请求的 orchestration 权限。Parent 可以创建 worker invocation，但 worker 不能获得相同的创建权限。Parent binding 的 `max_children` 是该 parent 同时存活 worker invocation 的总上限；未配置时为 24，并跨所有允许的 role 统一计数。
...
7. 可以创建多个相同 role 的 worker agent 实例，直到触达 role/profile/provider 并发上限或 parent 的 `max_children` 总量上限；默认分别为 6 和 24。

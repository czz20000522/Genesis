...
- Problem: The current context compaction path proves the owner boundary and runtime loop: provider usage can trigger compaction, the ledger records started/completed/failed events, future provider context receives a summary plus a recent complete-turn tail, user timelines hide the internal summary, and failed compaction can retry on a later eligible turn. The implementation is still a deliberately small spike. The OpenAI-compatible gateway already normalizes provider-reported input/output/total usage plus DeepSeek-style `prompt_cache_hit_tokens` and `prompt_cache_miss_tokens`; compaction must use those provider facts rather than a kernel-local tokenizer or character estimate. The remaining gap is provider/gateway-owned token accounting for model-visible context fragments or exchange boundaries before implementing token-budgeted tail selection, plus dedicated compaction backoff, tool-call/result pair accounting, compaction economics, and cache-stability analysis.

- Suggestion: Refine the owner implementation after the kernel closed loop stabilizes. Use provider-normalized usage plus model context metadata for threshold decisions, add gateway-owned token accounting for model-visible fragments where the provider can supply exact usage facts, keep any future token-budgeted recent-tail selector aligned with whole model-visible exchange boundaries, add bounded retry/backoff evidence for summarizer failures, and record enough internal metrics to explain cache hit/miss behavior without exposing summaries to user timelines.

- Evidence: `internal/kernel/context_compaction.go` writes `context.compaction.started`, `context.compaction.completed`, and `context.compaction.failed` events, projects future context from the latest completed summary, and preserves a configured minimum number of complete recent conversation turns. `internal/kernel/projections.go` renders only fixed compaction notices in `UITimeline`. `internal/kernel/openai_compatible.go` normalizes provider input/output/total usage and DeepSeek-style cache hit/miss fields into kernel-owned usage fields.

- Verification: Current guard tests cover summary-plus-tail model context, hidden user timeline summary, failed compaction evidence, retry on a later eligible turn, and OpenAI-compatible cache usage normalization. Future retirement requires tests for provider/gateway-owned token accounting, token-budgeted tail selection without local estimation, tool pair boundary preservation, compaction failure backoff, and cache-stability metrics.

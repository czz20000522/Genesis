...
- Design: `docs/applications/connector-source-supervisor-design.md`; broader
  connector design: `docs/applications/application-connector-runtime-design.md`.
- Kernel/owner pressure: connector source verification, inbound dedupe, session
  ...
  ownership.
- Gap: The smoke path now has deterministic adapter NDJSON ingestion, a real `lark-cli event consume im.message.receive_v1` source driver, durable connector-local source failure records with redacted diagnostics for malformed Feishu source events and recoverable source runtime errors, connector-local `SourceRun` / `SourceAttempt` / `SourceCursor` state, an operator-run `genesis-ingress feishu-probe` installed-adapter readiness report, and an opt-in `--deliver-final` path that turns kernel final text into one connector-owned send-message outbox item. The real source driver still marks source validation as `unchecked` because source readiness does not imply event verification, and webhook/signature/source verification is not yet a durable connector fact. Remaining production gap is full long-running source supervision, credential/profile refresh posture, token/profile refresh integration, and moving the hardcoded `lark-cli event consume` command shape behind connector driver configuration or an external adapter process.
- Next slice: Product smoke must use the Genesis profile (`--profile genesis`); Codex developer-originated test messages may still use the Codex profile as the sender. If local `lark-cli` resolves to an npm `.cmd`/`.ps1` shim or extensionless shell script and no direct binary is configured, production delivery must use a real binary or `connector_command` external adapter process instead of `command_template`. The current in-code Feishu event command and explicit `--source-attempts`/`--source-backoff` retry wrapper are smoke slices, not the final connector protocol or process supervisor. The next implementation cut should move source command shape behind source adapter driver configuration or an external adapter process, not add more Feishu-specific command knowledge to connector runtime code.
- Evidence: `genesis-ingress feishu-listen --stdin-jsonl --profile ...` covers automated smoke and dedupe. `FeishuEventSourceConfig` now constructs an explicit-profile `lark-cli event consume im.message.receive_v1 --as bot` command, rejects non-direct binaries, redacts stderr diagnostics, maps flattened Feishu message events into connector-owned `ExternalEvent` values before `/turn`, records malformed source events as redacted diagnostics in `FileSourceFailureStore` before kernel submission, and can ignore configured sender ids before kernel submission to avoid bot self-reply loops. `FileInboundStore`, `FileSourceFailureStore`, `FileSourceSupervisorStore`, and `FileOutboxStore` serialize file-backed load-modify-write operations across process-local instances during smoke use. `ConsumeFeishuEventSourceWithRetry` and `genesis-ingress feishu-listen --source-attempts ... --source-backoff ... --source-supervisor-state ...` record recoverable source runtime failures, blocked source readiness, attempts, and cursor state locally and retry only after valid source configuration. `genesis-console inspect --source-supervisor-state ...` projects connector-local source runs, attempts, and cursors without kernel mutation. `genesis-ingress feishu-probe --profile ... --lark-cli ...` validates the event-source and final-delivery command surfaces without starting the listener, sending a message, or calling kernel. The opt-in `--deliver-final` path uses connector outbox delivery rather than kernel Feishu logic.
- Verification: Remaining verification must prove unauthenticated real Feishu source events fail before kernel submission and become connector-local source records, verified source status is only emitted after durable source verification exists, repeated Feishu events dedupe before kernel turn submission and reply delivery, inbound retry exhaustion only affects connector request state, external Feishu identity does not set kernel authority, missing profile/credential fails before external source or send actions, and production listener supervision recovers from transient listener failures without creating a kernel Feishu owner.

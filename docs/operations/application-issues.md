...
- Current configuration slice: make real Feishu listener startup require the
  ...
  profile/identity values before source start. When the same binding disables
  unbound chats, its non-empty chat list is enforced by the generic
  source-frame admission boundary before request mapping or kernel submission;
  rejected threads record connector-local `source_policy_rejected` without raw
  thread ids. The generic runtime remains adapter-neutral; a later mail,
  WeChat, or QQ adapter may reuse the enablement/lifecycle boundary without
  forcing identical vendor settings.

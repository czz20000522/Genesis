...
- Retired in: connector file store integrity implementation change set.
- Requirement: `docs/applications/connector-delivery-state-machine-requirement.md`.
- Design: `docs/applications/application-connector-runtime-design.md`.
- Fix summary: `FileOutboxStore` now serializes load-modify-write operations with a connector-local lock file and reloads the latest on-disk payload under that lock before mutating or reading state. Separate store instances no longer overwrite independent enqueue writes, receipt appends, or delivery claims made by another console/worker process.
- Boundary evidence: The fix stays inside `internal/applications/connector_runtime`. It does not move connector state into kernel, does not let console commands rewrite kernel facts, and does not make the JSON file format a permanent product truth. The store remains connector-local infrastructure that can be retired in favor of a stronger production store.
- Verification: `TestFileOutboxStoreConcurrentInstancesPreserveIndependentEnqueues` and `TestFileOutboxStoreConcurrentInstancesPreserveReceiptHistory` first failed against the in-process-only mutex implementation, then passed after lock-file serialization. `TestFileOutboxStoreConcurrentInstancesDoNotClaimSameItem` proves two independent workers cannot claim the same queued item through stale snapshots. `go test ./internal/applications/connector_runtime -run "TestFileOutboxStoreConcurrentInstances" -count=1` and `go test ./internal/applications/connector_runtime -count=1` passed.
- Residual risk: This retires the bounded cross-process smoke integrity gap only. The file-backed store still has local-file operational limits and should be replaced, not endlessly patched, if production usage requires high-volume workers, durable database queries, crash-stale lock recovery, or multi-host operation.

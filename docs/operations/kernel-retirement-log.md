...
- `KERNEL-CONTEXT-COMPACTION-ADMISSION-ATOMICITY-20260629`: retired because manual compaction and `/turn` admission now share the `activeTurns` session ownership path before ledger mutation or history rewrite, with regression coverage for active-turn refusal and compaction-owned session turn refusal; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run "ManualCompaction|InterruptSession|Compaction" -count=1` and `go test ./internal/kernel -count=1`.
- `KERNEL-FEISHU-INBOUND-CONTEXT-QUALITY-20260627`: retired because manual and auto context compaction now share the kernel/session compaction runner, manual compaction is an unversioned idle-only control command with structured running-turn refusal, fake Feishu source-command long-session pressure proves connectors submit requests without owning compaction truth, and debug/timeline projections expose bounded context-quality diagnostics without leaking compaction summaries; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run "Compaction|Pressure|Debug|Timeline|ContextInspection" -count=1`, `go test ./internal/applications/connector_runtime -count=1`, `go test ./cmd/genesis-ingress ./cmd/genesis-feishu-source-adapter -count=1`, `go test ./internal/kernel -count=1`, `go test ./... -count=1`, `go build ./...`, and `git diff --check`.
- `KERNEL-PROVIDER-FAKE-PRODUCTION-GUARD-20260626`: retired because `genesisd` now resolves empty provider names through Genesis config, blocks `-provider fake` as `provider_fake_lab_only` unless explicit lab fake mode is enabled, keeps OpenAI-compatible missing keys as structured `provider_api_key_missing`, and live LLM acceptance rejects fake providers at both `/ready` and `/turn`; verified by the retiring commit tests and build.
- `KERNEL-MODEL-PROVIDER-ADAPTER-BOUNDARY-20260626`: retired because provider presets now write adapter bindings into model profiles, resolver carries the binding into OpenAI-compatible transport config, DeepSeek hidden reasoning is only discarded under explicit adapter policy while generic transport rejects unsupported vendor fields, SCNet setup resolves as an adapter profile over the shared transport, and architecture guards keep vendor wire terms out of generic kernel/provider DTOs; verified by `go test ./cmd/genesisctl -count=1` and `go test ./internal/kernel -run "ProviderSetup|ProviderAdapter|OpenAICompatible|ModelConfig|ArchitectureBoundary" -count=1`.
- `KERNEL-MODEL-PROVIDER-SETUP-WIZARD-20260626`: retired because `genesisctl provider use` now expands DeepSeek and SCNet presets into the existing Model Gateway setup path, `provider rotate-key` rotates the active credential without repeating provider metadata, and tests cover dry-run, secret non-leakage, unknown presets, shared config preservation, SCNet contract shape, and low-level `provider-setup` preservation; verified by `go test ./cmd/genesisctl -count=1` and `go test ./internal/kernel -run "ProviderSetup|ModelConfig|OpenAICompatibleConfig|LocalSecret|BlockedProvider" -count=1`.
- `KERNEL-MATERIAL-SOURCE-SNAPSHOT-BUDGET-LEASE-20260626`: retired because `SourceSnapshotPolicy` now governs source file count, per-file bytes, total uncompressed bytes, tree limits, and read limits with capabilities inspection; the pleiades pressure zip is admitted by default while explicit low budgets still fail closed, verified by `go test ./internal/kernel/resource -run "SourceSnapshot.*Budget|Pressure|RejectsUnsafe|ParallelReads|FileRefs" -count=1`, `go test ./internal/kernel -run "SourceSnapshotPolicyIsInspectableRuntimeLimit|MaterialSourceSnapshotPersistenceIsDeclaredProcessLifetimeOnly" -count=1`, and live `/materials/intake` smoke against `D:\User\Desktop\pleiades-anisop.zip`.
- `KERNEL-SOURCE-SNAPSHOT-PURE-READ-RACE-20260626`: retired because source file handles are generated at intake, `SourceTree` no longer mutates registry state during tool execution, archive changes do not expand admitted file authority, and source registry concurrent reads are race-clean under `go test -race ./internal/kernel/resource -run "TestSourceSnapshotParallelReadsDoNotRaceOrMutateRegistry" -count=1`.
- `KERNEL-MATERIAL-SOURCE-SNAPSHOT-DURABLE-OWNER-FACT-20260626`: retired by explicitly marking current source snapshot persistence as `not_ready/process_lifetime_only` in capabilities and testing restart behavior; durable source owner indexing remains future work rather than an implied recovery guarantee.
...
- `KERNEL-BUDGET-GUARD-CLASSIFICATION-20260625`: retired because capabilities/context inspection now expose classified runtime limits for BudgetLease, shell timeout policy, hard safety guards, provider retry/repair caps, and projection/output caps while model-visible schemas keep those controls hidden; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run "Shell|Timeout|Budget|ToolLoop|Capabilities|ContextInspection" -count=1` and `go test ./internal/kernel -count=1`.
- `KERNEL-REFERENCE-MODEL-FOUNDATION-20260625`: retired because text resources now project owner-local `ReferenceDescriptor` records with `available_operations=["read_text"]`, `resource_read` revalidates read_text admission at prepare and execute time, runtime handles/internal refs/host paths fail closed without filesystem fallback, and the model-visible tool surface remains typed without universal `ref_read`; fixing commits `80747182a`, `c08f21d0f`, `6d4db0177`, and `6ae6ccfa2`, verified by `go test ./internal/kernel/resource -count=1`, `go test ./internal/kernel -run "ResourceRead|ContextHydration|ToolScheduling|ArchitectureBoundary" -count=1`, `go test ./internal/kernel -count=1`, `go test ./... -count=1`, `go build ./...`, and `git diff --check`.
- `KERNEL-CONTEXT-HYDRATION-TURN-SCOPE-20260625`: retired because `context.admit_resource` now records `admission_result=admitted/refused`, non-empty `turn_id` is refused with `refusal_reason_class=scope_violation`, provider context consumes only session-scoped `context.hydration.admitted` facts once on the next turn, and refused hydration never becomes timeline/tool/prompt content; fixing evidence is in the retiring Lore commit, verified by the required hydration/resource/kernel/build checks.
- `KERNEL-CONTEXT-HYDRATION-DOC-DRIFT-20260625`: retired because the resource-read requirement/design now describe implemented generic session-scoped hydration admission, explicitly refuse unsupported post-submit turn-scoped hydration, and limit future work to object store, attachment ingest, OCR/binary rendering, vector index, freshness policy, and richer selection policy; fixing evidence is in the retiring Lore commit, verified by the stale-phrase scan.
- `KERNEL-CONTEXT-RESOURCE-HYDRATION-20260625`: retired because generic `context.admit_resource` now records `context.hydration.accepted/rejected` owner facts, accepted bounded text resources enter provider context only as `hydrated_context`, rejected/missing/unsupported/oversize/scope-mismatch admissions do not splice prompt text, default skill context remains metadata-only, forbidden `skill.read`/`read_skill` surfaces stay absent, and context inspection carries source owner/ref/hash/mime/byte/truncation evidence; fixing evidence is in the retiring Lore commit, aligned with Codex bounded typed context fragments and Reasonix capped tagged context blocks, verified by `go test ./internal/kernel -run "ContextHydration|ResourceRead|SkillCatalog|ArchitectureBoundaryNoSkillSpecificHydrationTools|ArchitectureBoundaryOwnerDTOsLiveInNamedFiles|ArchitectureBoundaryResourceOwnerHasSubpackageTypes|ContextInspection|Timeline" -count=1`, `go test ./internal/kernel/resource -count=1`, `go test ./internal/kernel -count=1`, `go test ./... -count=1`, `go build ./...`, and `git diff --check`. Future work remains production object storage, attachment ingestion, OCR/binary rendering, vector search, and richer resource freshness policy.
- `KERNEL-USER-CONTENT-REDACTION-RESET-20260625`: retired because ordinary local session, timeline, context inspection, shell/job, resource, memory, and compaction projections no longer use global irreversible `[REDACTED]` replacement for user-owned content; credential/env safety remains structural at provider-command/env/readiness boundaries, verified by `go test ./internal/kernel -run "Redaction|Content|ResourceRead|Shell|Memory|Budget|Environment|Projection" -count=1`, `go test ./internal/kernel -count=1`, `go test ./... -count=1`, `go build ./...`, and `git diff --check`.
- `KERNEL-REFERENCE-BEHAVIOR-RED-TEST-MATRIX-20260625`: retired because kernel implementation plans with reference scans now must name `Reference Behavior Red Tests`, the template/process and owner-structure requirement/design say so, current kernel plans are backfilled, and a structural architecture guard enforces the section without judging prose; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run TestArchitectureBoundaryKernelImplementationPlansNameReferenceBehaviorRedTests -count=1`.
- `KERNEL-TOOL-BATCH-PARALLEL-SIGNAL-20260625`: retired because `ToolExecutionBatch.Parallel` now reports actual current concurrent-runner eligibility, process-IO compatible grouping stays serial until a real executor/replay contract exists, and reference-derived red tests lock the distinction; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run "TestPlanToolExecutionBatchesSerializesSameHandleProcessIO|TestPlanToolExecutionBatchesParallelFlagMatchesConcurrentEligibility|TestExecuteToolBatchesKeepsProcessIOBatchSerialByDefault|TestExecuteToolBatchesRunsPureReadBatchConcurrently" -count=1`.
- `KERNEL-PROVIDER-COMMAND-STRICT-RESPONSE-20260625`: retired because Genesis-owned `provider_command` responses now use strict top-level and nested tool-call decoding, reject hidden/control-plane drift before tool admission, preserve valid final/tool-call paths, and keep OpenAI-compatible vendor-native extras tolerant; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run "TestCommandProviderStrictlyRejectsUnknownResponseFields|TestCommandProviderStrictShapeFailureDoesNotAdmitToolCall|TestCommandProviderStrictDecoderPreservesValidResponses|TestOpenAICompatibleProviderToleratesVendorNativeExtraFields|TestCommandProvider|TestOpenAICompatibleProvider" -count=1`.
- `KERNEL-BUDGET-LEASE-20260625`: retired because model tool-round execution is now governed by normalized kernel `BudgetLease` policy with an allowed ceiling, inspectable capabilities/context projections, non-model-visible tool schemas, and `turn.paused` evidence reporting the effective lease; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run "TestSubmitTurnUsesConfiguredBudgetLeaseForToolRounds|TestSubmitTurnNormalizesZeroBudgetLeaseToDefault|TestBudgetLeaseIsInspectableButNotModelVisible|TestBudgetLeaseClampsConfiguredBudgetToCeiling|TestBudgetDocsSeparateExecutionLeasesFromHardSafetyCaps" -count=1` and `go test ./internal/kernel -count=1`.
- `KERNEL-APPROVAL-APPROVED-RECOVERY-EVIDENCE-20260625`: retired because approval replay now has direct crash-window regression coverage proving an already-persisted `approval.approved` fact can recover the frozen effect exactly once while policy drift still fails closed; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run "TestApprovalApprovedCrashWindowReplay" -count=1`.
- `KERNEL-SANDBOX-APPROVAL-NEXT-20260623`: retired because sandbox readiness now records ready/unavailable pre-effect evidence, approval owner facts cover requested/approved/denied/expired decisions for frozen effect requests, the minimal HTTP control surface only submits decision commands, and approved effects resume through the kernel shell path without model-authored control fields; fixing evidence is in the retiring Lore commit, verified by `go test ./internal/kernel -run "TestApproval|TestUnavailableSandbox|TestSubmitTurnBlocks|TestSubmitTurnReturnsRepairFeedbackForUnknownModelToolArgumentFields|TestArchitectureBoundaryApproval|TestArchitectureBoundarySandbox" -count=1`, `go test ./internal/kernel -count=1`, `go test ./... -count=1`, `go build ./...`, and `git diff --check`.
- `KERNEL-CONTEXT-COMPACTION-STUCK-GUARD-20260625`: retired because auto compaction now defers with typed stuck-window evidence and a timeline notice after adjacent successful auto compactions still leave provider-reported input over the limit, while healthy separated compactions continue; design commit `0556a2d57`, fixing commit `d102b969b`, verified by `go test ./internal/kernel -run "TestAutoCompaction|TestCompactionSource|TestModelGatewayRecordsProviderBackedContextAccounting|TestModelGatewayAccountsToolRoundBoundaries" -count=1`, `go test ./internal/kernel -count=1`, `go test ./... -count=1`, `go build ./...`, and `git diff --check`.
- `KERNEL-FOREGROUND-PROCESS-TREE-CLEANUP-GUARD-20260625`: retired because foreground shell timeout and user interruption now have platform-aware descendant marker regression tests proving kernel termination evidence is not followed by descendant side effects; fixing commit `577624636`, verified by `go test ./internal/kernel -run "Test.*Foreground.*ProcessTree|Test.*Foreground.*Timeout|Test.*InterruptSessionDuringForegroundShell" -count=1`.
- `KERNEL-TOOL-LOOP-STORM-GUARD-PROGRESS-RESET-20260625`: retired because repeated successful write-like storm counters now reset after a different trusted write-like success while the unchanged A/A/A path still blocks before the third same-effect side effect; fixing commit `6274cfa5e`, verified by `go test ./internal/kernel -run "TestToolLoop.*Storm|TestToolLoop.*Repeat|Test.*Tool.*Loop" -count=1`, `go test ./internal/kernel -count=1`, `go test ./... -count=1`, `go build ./...`, and `git diff --check`.
- `KERNEL-TOOL-PARALLEL-FAILURE-SEMANTICS-20260625`: retired because pure-read parallel execution now preserves model-actionable per-call failures as ordered `tool.result` facts while duplicate/missing/unexpected runner results and interrupted batches produce kernel-owned terminal evidence without forged sibling results; fixing commit `4ddfaa6c6`, verified by `go test ./internal/kernel -run "TestExecuteToolBatches|TestPlanToolExecutionBatches|TestToolScheduling" -count=1` and `go test ./internal/kernel -count=1`.
- `KERNEL-UI-TIMELINE-DETAIL-INFORMATION-DENSITY-20260625`: retired because `UITimelineDetail` now uses an enriched sanitized read model with command/argument preview, visible output, duration, and truncation metadata while default `UITimeline` remains compact and redacted; fixing commit `4ddfaa6c6`, verified by `go test ./internal/kernel -run "TestUITimeline|TestHTTP.*Timeline|Test.*Timeline.*Detail" -count=1` and `go test ./internal/kernel -count=1`.
- `KERNEL-TOOL-LOOP-STORM-GUARD-20260625`: retired because Tool Runtime now augments repeated same-error tool results with model-visible `loop_guard` feedback, blocks the third repeated successful write-like shell effect before execution, and resets guards after real progress; fixing commit `59220dd43`, verified by `go test ./internal/kernel -count=1`.
- `KERNEL-TOOL-LOOP-PAUSE-RESUME-20260625`: retired because round-budget exhaustion now records `turn.paused` instead of `turn.failed`, preserves committed tool rounds, replays paused idempotency evidence, and exposes the paused frontier to same-session continuation context without kernel ids; fixing commit `59220dd43`, verified by `go test ./internal/kernel -count=1`.

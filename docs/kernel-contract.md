...
External skill packages are user-space assets, not kernel applications. The kernel may be configured with explicit skill roots and may scan `SKILL.md` front matter into a read-only skill catalog for model context. This catalog is metadata only: name, description, and instruction path. Skill metadata is treated as untrusted before injection; authority-shaped, prompt-injection-shaped, hidden-control, or secret-shaped metadata is excluded rather than repaired into context. The catalog does not grant authority, load full skill bodies into every turn, execute external CLIs, or create Feishu, email, calendar, document, or channel-specific kernel code.

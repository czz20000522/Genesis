...
Context compaction is a Model Gateway and ledger responsibility. When a configured context window and auto-compact threshold are reached by provider-reported input usage, the kernel may summarize the older completed conversation region, write `context.compaction.started` and `context.compaction.completed` ledger events, and keep subsequent provider projections to the latest summary plus a recent verbatim tail. The current tail boundary is a minimum number of complete conversation turns. Token-budgeted tail selection must be based on provider/gateway token accounting, not a kernel-local tokenizer or character estimate, and must still avoid splitting a user/assistant turn. A failed compaction writes `context.compaction.failed` with a structured reason and leaves the previous provider projection in place; the completed user response must not be turned into a model failure. The compaction source and result are auditable in the ledger, but shells and external daemons must not perform their own truncation, summary, or replay rewriting. User-facing timelines may show only progress/completion/failure notices for compaction and must not render the internal summary as a chat message.

...
The initial tool loop is deliberately narrow. OpenAI-compatible providers receive the canonical `shell.exec` descriptor. If the provider returns a `shell.exec` tool call, Genesis executes it through the existing Tool System, writes turn-scoped operation events to the ledger, and returns the redacted operation projection to the provider as tool evidence. Unsupported tool requests, including `skill.read`, return repair feedback without executing effects. The provider then returns the final assistant text.

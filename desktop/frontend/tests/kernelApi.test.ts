...
assert.equal(conversationSource.includes('回合 ${props.lastTurn.turn_id} 已提交'), false, 'ConversationPane must not expose kernel turn identifiers as chat status')
assert.equal(conversationSource.includes('当前模型'), true, 'ConversationPane must keep the current session model readable beside the composer')
assert.equal(conversationSource.includes("retryText ? '重试这次对话' : '重新连接'"), true, 'ConversationPane must offer a useful recovery action when the desktop loses the kernel connection')
assert.equal(conversationSource.includes('<p class="send-failure-description">{{ error }}</p>'), true, 'ConversationPane must keep the recovery error visible beside its retry action')
assert.equal(conversationSource.includes("row.terminalOutcome === 'succeeded'"), true, 'ConversationPane must recognize the kernel succeeded outcome when folding completed processing')
